  ...
  return (
    <div>
      <div className="relative aspect-[4/3] overflow-hidden rounded-3xl bg-slate-200 shadow-sm">
        <Image src={config.beforeAfter.beforeImage} alt={t("clinic.labels.comparisonBeforeLabel", config.labels.comparisonBeforeLabel)} fill sizes="(min-width: 1024px) 50vw, 100vw" className="object-cover" />
        ...
        <span className="absolute left-4 top-4 rounded-full bg-slate-950/75 px-3 py-1 text-xs font-semibold text-white">{t("clinic.labels.comparisonAfterLabel", config.labels.comparisonAfterLabel)}</span>
        <span className="absolute right-4 top-4 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-slate-900">{t("clinic.labels.comparisonBeforeLabel", config.labels.comparisonBeforeLabel)}</span>
        <input
          type="range"
          min="0"
          max="100"
          ...
          aria-label={t("clinic.labels.comparisonControlLabel", config.labels.comparisonControlLabel)}
