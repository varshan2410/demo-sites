...
export default function WhatsAppFloat({ config }: { config: { whatsapp: WhatsAppConfig } }) {
  ...
  return (
    <div className="whatsapp-float fixed bottom-5 right-5 z-50 sm:bottom-6 sm:right-6">
      <a
        href={url}
        target="_blank"
        rel="noopener noreferrer"
        aria-label={config.whatsapp.ariaLabel}
        className="ui-button flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg"
      >
