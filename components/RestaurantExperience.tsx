...
export default function RestaurantExperience({ config }: { config: RestaurantConfig }) {
  const [category, setCategory] = useState(config.labels.menuAllLabel);
  const [cart, setCart] = useState<Record<string, number>>({});
  const [reservation, setReservation] = useState("");
  const categories = [config.labels.menuAllLabel, ...Array.from(new Set(config.menu.map((item) => item.category)))];
  const visibleMenu = category === config.labels.menuAllLabel ? config.menu : config.menu.filter((item) => item.category === category);
    ...
    <>
      <section id="home" className="relative isolate min-h-[620px] overflow-hidden px-6 py-24 text-white md:py-36">
        ...
        <div className="relative mx-auto flex min-h-[400px] max-w-6xl items-end">
          <div className="max-w-2xl">
            ...
            <p className="mt-6 max-w-xl text-lg leading-8 text-stone-200">{config.hero.subtitle}</p>
            <a href="#menu" className="mt-8 inline-flex rounded-full bg-white px-6 py-3 font-semibold text-stone-950">{config.hero.cta}</a>
          </div>
        </div>
      </section>
      ...
      <section id="menu" className="px-6 py-16 md:py-24">
        <div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1fr_320px]">
          <div>
            <p className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: config.theme.primary }}>{config.labels.menuEyebrow}</p>
            <h2 className="mt-3 text-3xl font-semibold tracking-tight dark:text-white md:text-4xl">{config.labels.menuTitle}</h2>
            <div className="mt-7 flex flex-wrap gap-2">
              {categories.map((item) => <button key={item} type="button" onClick={() => setCategory(item)} className="rounded-full px-4 py-2 text-sm font-semibold" style={category === item ? { backgroundColor: config.theme.primary, color: "white" } : { backgroundColor: "#f1f5f9", color: "#334155" }}>{item}</button>)}
            </div>
            <div className="mt-8 grid gap-4 sm:grid-cols-2">
              {visibleMenu.map((item) => (
                <article key={item.id} className="rounded-2xl border border-slate-200 p-5 dark:border-slate-800">
                  <div className="flex items-start justify-between gap-4"><h3 className="font-semibold dark:text-white">{item.name}</h3><span className="whitespace-nowrap font-semibold dark:text-white">LKR {item.priceLKR.toLocaleString()}</span></div>
          ...
          <aside className="h-fit rounded-3xl bg-stone-100 p-6 dark:bg-slate-900 lg:sticky lg:top-24">
            <h2 className="text-xl font-semibold dark:text-white">{config.labels.orderTitle}</h2>
            {cartItems.length ? <div className="mt-5 space-y-4">{cartItems.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 text-sm"><div><p className="font-medium dark:text-white">{item.name}</p><p className="text-slate-500">LKR {(item.priceLKR * cart[item.id]).toLocaleString()}</p></div><div className="flex items-center gap-2"><button type="button" onClick={() => updateCart(item.id, -1)} className="h-7 w-7 rounded-full border dark:border-slate-700">−</button><span>{cart[item.id]}</span><button type="button" onClick={() => updateCart(item.id, 1)} className="h-7 w-7 rounded-full border dark:border-slate-700">+</button></div></div>)}</div> : <p className="mt-4 text-sm text-slate-600 dark:text-slate-300">{config.labels.orderEmptyLabel}</p>}
            <div className="mt-6 border-t border-stone-300 pt-4 dark:border-slate-700"><div className="flex justify-between font-semibold dark:text-white"><span>{config.labels.orderTotalLabel}</span><span>LKR {total.toLocaleString()}</span></div><a href={"https://wa.me/" + config.whatsapp.number + "?text=" + orderMessage} target="_blank" rel="noopener noreferrer" className="mt-4 block rounded-full px-4 py-3 text-center text-sm font-semibold text-white" style={{ backgroundColor: config.theme.primary }}>{config.labels.orderWhatsAppLabel}</a></div>
          </aside>
        </div>
      </section>
