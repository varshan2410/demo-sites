    ...
    <>
      <section id="home" className="relative isolate min-h-[660px] overflow-hidden px-6 py-24 text-white md:py-36">
        ...
        <div className="relative mx-auto flex min-h-[430px] max-w-6xl items-end">
          <div className="max-w-2xl">
            ...
            <h1 className="mt-5 text-5xl font-semibold leading-[1.02] tracking-tight md:text-7xl">{config.hero.title}</h1>
            <p className="mt-6 max-w-lg text-lg leading-8 text-stone-200">{config.hero.subtitle}</p>
      ...
      <section id="rooms" className="px-6 py-16 md:py-24">
        <div className="mx-auto max-w-6xl">
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: config.theme.primary }}>{config.labels.roomsEyebrow}</p>
              <h2 className="mt-3 text-3xl font-semibold tracking-tight dark:text-white md:text-4xl">{config.labels.roomsTitle}</h2>
            </div>
            <label className="text-sm font-medium text-slate-700 dark:text-slate-200">
              {config.labels.currencyLabel}
          ...
          <div className="mt-10 grid gap-5 md:grid-cols-3">
            {config.rooms.map((room, index) => (
              <article key={room.id} className="overflow-hidden rounded-3xl border border-stone-200 bg-stone-50 p-6 dark:border-slate-800 dark:bg-slate-900">
                ...
                <h3 className="mt-2 text-xl font-semibold dark:text-white">{room.name}</h3>
                ...
                <p className="mt-6 text-xl font-semibold dark:text-white">{formatPrice(room.rateLKR)} <span className="text-sm font-normal text-slate-500">{config.labels.roomsRateSuffix}</span></p>
              </article>
            ))}
          </div>
          <p className="mt-4 text-xs text-slate-500 dark:text-slate-400">{config.labels.indicativeRateLabel}</p>
        </div>
      </section>

      <section id="experiences" className="bg-stone-100 px-6 py-16 dark:bg-slate-900 md:py-24">
        <div className="mx-auto max-w-6xl">
          <p className="text-xs font-bold uppercase tracking-[0.2em]" style={{ color: config.theme.primary }}>{config.labels.attractionsEyebrow}</p>
          <h2 className="mt-3 text-3xl font-semibold tracking-tight dark:text-white md:text-4xl">{config.labels.attractionsTitle}</h2>
          <div className="mt-10 grid gap-5 md:grid-cols-3">
      ...
      <section id="availability" className="px-6 py-16 md:py-24">
        <div className="mx-auto max-w-2xl">
          <p className="rounded-full px-4 py-2 text-center text-sm font-semibold text-white" style={{ backgroundColor: config.theme.primary }}>{config.labels.directBookingMessage}</p>
          {enquiry ? (
            <div id="hotel-voucher" className="mt-6 rounded-3xl border border-stone-200 p-8 text-center dark:border-slate-800">
              <h2 className="text-2xl font-semibold dark:text-white">{config.labels.confirmationTitle}</h2>
              ...
              <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
                <a href={"https://wa.me/" + config.whatsapp.number + "?text=" + encodeURIComponent(whatsappMessage)} target="_blank" rel="noopener noreferrer" className="rounded-full px-5 py-3 text-sm font-semibold text-white" style={{ backgroundColor: config.theme.primary }}>{config.labels.confirmationWhatsAppLabel}</a>
                <button type="button" onClick={() => window.print()} className="rounded-full border border-slate-300 px-5 py-3 text-sm font-semibold dark:border-slate-700 dark:text-slate-200">{config.labels.printVoucherLabel}</button>
              </div>
            </div>
          ) : (
            <div className="mt-6 rounded-3xl bg-stone-50 p-6 dark:bg-slate-900 sm:p-8">
              <h2 className="text-2xl font-semibold dark:text-white">{config.labels.availabilityTitle}</h2>
              <form onSubmit={submitEnquiry} className="mt-6 grid gap-4 sm:grid-cols-2">
                <label className="text-sm font-medium dark:text-slate-200">{config.labels.checkInLabel}<input required name="check-in" type="date" className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>
                <label className="text-sm font-medium dark:text-slate-200">{config.labels.checkOutLabel}<input required name="check-out" type="date" className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>
                <label className="text-sm font-medium dark:text-slate-200">{config.labels.guestsLabel}<input required name="guests" type="number" min="1" max="6" defaultValue="2" className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>
                <label className="text-sm font-medium dark:text-slate-200">{config.labels.roomLabel}<select required name="room" defaultValue="" className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-white"><option value="" disabled>{config.labels.roomPlaceholder}</option>{config.rooms.map((room) => <option key={room.id} value={room.id}>{room.name}</option>)}</select></label>
                <label className="text-sm font-medium dark:text-slate-200">{config.labels.enquiryNameLabel}<input required name="name" type="text" className="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-slate-950 dark:border-slate-700 dark:bg-slate-950 dark:text-white" /></label>
                ...
                <button type="submit" className="sm:col-span-2 rounded-full px-5 py-3 font-semibold text-white" style={{ backgroundColor: config.theme.primary }}>{config.labels.enquirySubmitLabel}</button>
              </form>
            </div>
          )}
        </div>
