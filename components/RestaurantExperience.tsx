...
export default function RestaurantExperience({ config: sourceConfig }: { config: RestaurantConfig }) {
  ...
  const orderMessage = cartItems.length
    ? [
        `Hello ${config.siteName}!`,
        "",
        "I would like to place the following order:",
        "",
        ...cartItems.map((item) => `• ${cart[item.id]} × ${item.name} — LKR ${(cart[item.id] * item.priceLKR).toLocaleString()}`),
        "",
