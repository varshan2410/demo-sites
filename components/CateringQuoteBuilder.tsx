...
export default function CateringQuoteBuilder({ packages, contact, whatsapp, theme, siteName, labels }: { packages: CateringPackage[]; contact: ContactConfig; whatsapp: WhatsAppConfig; theme: ThemeConfig; siteName: string; labels: QuoteLabels }) {
  ...
  return (
    <>
      ...
      {isMounted && isPrinting ? createPortal(<article id="catering-quote-receipt"><div className="receipt-card"><header className="receipt-header" style={{ backgroundColor: theme.primary }}><p style={{ margin: 0, fontSize: 12, fontWeight: 700, letterSpacing: ".12em", textTransform: "uppercase" }}>Catering quote</p><h1 style={{ margin: "8px 0 0", fontSize: 26 }}>{siteName}</h1><p style={{ margin: "6px 0 0", opacity: .86 }}>{contact.address}</p></header><div className="receipt-body"><p style={{ margin: "0 0 20px", fontSize: 15 }}>Estimated catering quote</p><div className="receipt-grid"><div><p className="receipt-label">Package</p><p className="receipt-value">{selectedPackage.name}</p></div><div><p className="receipt-label">Guests</p><p className="receipt-value">{guestCount}</p></div><div><p className="receipt-label">Rate</p><p className="receipt-value">LKR {selectedPackage.pricePerHeadLKR.toLocaleString()} per person</p></div><div><p className="receipt-label">Estimated total</p><p className="receipt-value">LKR {total.toLocaleString()}</p></div></div><footer className="receipt-footer">This is an estimated quote. Final selections, service charges and availability will be confirmed by the restaurant.<br />{contact.hours}</footer></div></div></article>, document.body) : null}
    </>
  );
}
